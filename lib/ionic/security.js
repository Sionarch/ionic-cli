...
var settings = {
  ...
  args: {
    '<command>': chalk.yellow('profiles list') + ', ' + chalk.yellow('profiles add "<name>"') + ', ' +
      chalk.yellow('credentials android') + ', ' + chalk.yellow('credentials ios') + ', or ' +
      chalk.yellow('pushcredentials ios') + ', or ' + chalk.yellow('pushcredentials android'),
    '[options]': ''
  },
  options: {
    ...
    '--cert|-c <path>': '(' + chalk.yellow('[push]credentials ios') +
      ') Specify the location of your .p12 file',
    '--cert-password|-p <password>': '(' + chalk.yellow('[push]credentials ios') +
    ...
    '--push-api-key|-a <path>': '(' + chalk.yellow('pushcredentials android') +
      ') Specify your FCM Server Key on Ionic Push'
  },
  isProjectTask: true
};
...
function run(ionic, argv) {
  ...
  case 'pushcredentials':
    return addPushCredentials(ionic, argv, dir, appId);
  }
...
function addSecurityCredentials(ionic, argv, dir, appId) {
  ...
  return Login.retrieveLogin()
    ...
    .then(function() {
      console.log('Added ' + platform + ' credentials to your Security Profile.');
    }, function(err) {
      if (typeof err === 'object') {
...
function addPushCredentials(ionic, argv, dir, appId) {
  ...
  return Login.retrieveLogin()
    ...
    .then(function(j) {
      ...
      if (platform === 'android') {
        if (typeof argv.s !== 'undefined') {
          argv['push-api-key'] = argv.k;
        }

        Prompt.prompt([
            { name: 'push-api-key', description: 'FCM Server Key:'.prompt, required: true }
        ], argv)
          .then(function(result) {
            q.resolve(result);
          })
        ...
        Prompt.prompt([
            { name: 'cert', description: 'Push Certificate File:'.prompt, required: true },
            { name: 'cert-password', description: 'Push Certificate Password:'.prompt, hidden: true, required: true }
        ], argv)
          .then(function(result) {
            q.resolve(result);
          })
    ...
    .then(function(result) {
      if (platform === 'android') {
        var fcmServerKey = result['push-api-key'];

        return Security.addAndroidPushCredentials(appId, jar, profile, fcmServerKey);
      } else if (platform === 'ios') {
        var certificateFilePath = path.resolve(expandTilde(result['cert']));
        var certificatePassword = result['cert-password'];
        var certificateFileStream = fs.createReadStream(certificateFilePath);

        return Security.addIOSPushCredentials(appId, jar, profile, certificateFileStream,
                                          certificatePassword);
      }
    ...
    .then(function() {
      console.log('Added ' + platform + ' push credentials to your Security Profile.');
    }, function(err) {
      if (typeof err === 'object') {
        if (err.type === 'CredentialsExist') {
          console.error(err.message);
